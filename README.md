...
Ollama (local AI, mistral:7b) — best quality, needs local setup
Template Engine (40+ empathetic responses) — always works, no dependencies
Crisis Safety (keyword detection) — immediate safety resources
Project Statistics
Metric	Value
Total rebuilds	3
Files created	45+
...
Key Lessons
Start with the deployment target. Flask → Vercel was a dead end. Next.js → Vercel is seamless.
Template engines are underrated. For emotional support, hand-crafted responses are more empathetic and reliable than any small local model.
Security first. Hardcoded API keys will be found and abused.
Build for failure. Every feature should work even when external services are down.
Simplify ruthlessly. 3 dependencies (next, react, tailwind) vs 15+ in the Flask version.
What's Inside
File	Purpose
...
Built with patience, persistence, and a lot of deep breaths. 
